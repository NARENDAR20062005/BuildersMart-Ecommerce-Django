...
def admin_view_booking_view(request):
    orders=models.Orders.objects.select_related('product', 'customer').order_by('-order_date')
    ordered_products=[]
    ordered_bys=[]
    for order in orders:
        ordered_product=models.Product.objects.all().filter(id=order.product.id)
...
def my_order_view(request):
    ...
    customer=models.Customer.objects.get(user_id=request.user.id)
    orders=models.Orders.objects.all().filter(customer_id = customer).order_by('-order_date')
    ordered_products=[]
    for order in orders:
        ordered_product=models.Product.objects.all().filter(id=order.product.id)
        ordered_products.append(ordered_product)
...
def download_invoice_view(request, orderID, productID):
    ...
    if pdf:
        ...
        # Send email
        subject = "Your Invoice from E-Commerce Site"
        message = "Dear {},\n\nThank you for your order. Please find attached your invoice.".format(request.user.username)
    ...
    return redirect('email_sent', orderID=orderID, productID=productID)
...
def edit_profile_view(request):
    ...
    # Initialize the forms with the user's and customer's current data
    userForm = forms.CustomerUserForm(instance=user)
...
def email_sent_view(request, orderID, productID):
    # You can use orderID and productID as needed here
    return render(request, 'ecom/emailsent.html')



@login_required(login_url='customerlogin')
@user_passes_test(is_customer)
def download1_invoice_view(request,orderID,productID):
    order=models.Orders.objects.get(id=orderID)
    product=models.Product.objects.get(id=productID)
    mydict={
        'orderDate':order.order_date,
    ...
    return render_to_pdf('ecom/download1_invoice.html',mydict)
